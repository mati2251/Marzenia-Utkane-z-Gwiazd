...
const allLinks = linkLabel.map(element => {
	return (
		<NavigationElement
			...
			key={element.toLink}
			className={styles.aside__navigationElement}
		>
			{element.label}
		</NavigationElement>
	)
		...
		<>
			<header className={styles.sidebar}>
				<div className={styles.header}>
					...
					<button
						className={styles.header__openLogoButton}
						onClick={() => {
							setAsideStyle(styles.asideOpen)
						}}
					>
						<MdMenu className={styles.header__openLogo}/>
					</button>
				</div>
			</header>
			<aside className={asideStyle}>
				<button
					className={styles.aside__closeMenuButton}
					onClick={() => {
